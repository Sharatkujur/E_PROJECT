<?php
$db = mysqli_connect('localhost', 'root', '', 'amwaydb');
...
function addCart() {
    ...
    if (isset($_GET['add_cart'])) {
        ...
        $p_id = isset($_GET['add_cart']) ? $_GET['add_cart'] : 0;
        $qty = isset($_POST['product_qty']) ? $_POST['product_qty'] : 1; // Default quantity to 1
        $size = isset($_POST['size']) ? $_POST['size'] : ''; // Default size to empty
        ...
        if (mysqli_num_rows($run_check) > 0) {
            ...
            // Add product to cart
            $query = "INSERT INTO cart (products_id, ip_add, qty, size, date, c_id) 
                      VALUES ('$p_id', '$ip_add', '$qty', '$size', NOW(), '$c_id')";
...
function cart_items() {
    ...
    $get_items = "SELECT cart.products_id, cart.qty, products.product_title, products.product_price, products.product_img1 
                  FROM cart 
    ...
    if ($countrows == 0) {
        ...
        while ($row_items = mysqli_fetch_array($run_items)) {
            ...
            $pro_price = $row_items['product_price'];
            $pro_img1 = $row_items['product_img1'];

            // Calculating the total price of the item
            $pro_total_p = $pro_price * $pro_qty;

            // Outputting the cart row with product details
            echo "
            <tr style='border-bottom: 0.5px solid #ebebeb'>
               ...
               <td class='cart-title first-row'>
                   <h5><a href='product.php?product_id=$p_id' style='color:black;font-weight:bold'>$pro_name</a></h5>
               </td>
               <td class='p-price first-row'>₹ $pro_price</td>
               <td class='qua-col first-row'>
                   <form action='shopping-cart.php' method='POST'>
                       <input type='hidden' name='product_id' value='$p_id'>
                       <input type='number' name='qty' value='$pro_qty' min='1' style='width: 50px;'>
               ...
               <td class='close-td first-row'><a href='shopping-cart.php?del=$p_id'><i class='ti-close' style='color:black'></i></a></td>
           </tr>    
           ";
        }
    }
